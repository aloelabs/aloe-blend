...
const CURRENT_BLOCK = 14439585;
const SECONDS_PER_BLOCK = 15;
const UNIV2_PAIR_ADDRESS = "0xb4e16d0168e52d35cacd2c6185b44281ec28c9dc";

const CHARM_ADDRESS = "0x9bF7B46C7aD5ab62034e9349Ab912C0345164322";
const VISOR_ADDRESS = "0x716bd8A7f8A44B010969A1825ae5658e7a18630D";
...
async function getPricePerShareV2(uniswapV2Pair, prices, symbol0, symbol1, decimals0, decimals1) {
  let res = await uniswapV2Pair.methods.getReserves().call();
  const inventory0 = new Big(res._reserve0);
  const inventory1 = new Big(res._reserve1);
  const totalSupply = new Big(await uniswapV2Pair.methods.totalSupply().call());
...
async function getPricePerShareCharmish(vault, prices, symbol0, symbol1, decimals0, decimals1) {
  let res = await vault.methods.getTotalAmounts().call();
  const inventory0 = new Big(res.total0.toString(10));
  const inventory1 = new Big(res.total1.toString(10));
  const totalSupply = new Big((await vault.methods.totalSupply().call()).toString(10));
...
async function getPricePerShareHistory(blendPoolAddress, blockFromWhichPollingBegins, blockInterval, blockAtWhichPollingEnds) {
  ...
  const uniswapV2Pair = new web3.eth.Contract(IUniswapV2Pool, UNIV2_PAIR_ADDRESS);

  let charmVault = null;
  if (CHARM_ADDRESS !== null) charmVault = new web3.eth.Contract(ICharmish, CHARM_ADDRESS);
  let visorVault = null;
  if (VISOR_ADDRESS !== null) visorVault = new web3.eth.Contract(ICharmish, VISOR_ADDRESS);
  ...
  for (let i = 0; i < N; i++) {
    ...
    // Uniswap V2 data
    const pricePerShareV2 = await getPricePerShareV2(uniswapV2Pair, prices, symbol0, symbol1, decimals0, decimals1);
    if (!pricePerShareV2) continue;

    // Competitor data
    let pricePerShareCharm;
    if (charmVault) {
      try {
        pricePerShareCharm = (await getPricePerShareCharmish(charmVault, prices, symbol0, symbol1, decimals0, decimals1)).toFixed(8);
      } catch {}
    }
    let pricePerShareVisor;
    if (visorVault) {
      try {
        pricePerShareVisor = (await getPricePerShareCharmish(visorVault, prices, symbol0, symbol1, decimals0, decimals1)).toFixed(8);
      } catch {}
    }
    ...
    const summary = {
      ...
      'pricePerShareV2': pricePerShareV2.toFixed(8),
      'pricePerShareCharm': pricePerShareCharm,
      'pricePerShareVisor': pricePerShareVisor,
    };
    console.log(summary);
